...
\end{prooftree}
%
% TODO: spremenjena implikacija
Tri pike \(\vdots\) predstavljajo, da iz \(A\) pokažemo \(B\). Na vrhu \(\cdot\) predstavlja prazn nabor premis. 
Navadno se pike ne piše in se pusti prostor nad črto prazen. Tu je napisana, da poudarimo, da nimamo začetnih pogojev. 
Ne predpostavimo, da \(A\) velja in potem pokažemo \(B\), temveč brez dodatnih predpostavk pokažemo, da iz \(A\) sledi \(B\). Končno pravilo označimo z \(\Rightarrow I^u\), da poudarimo, da implikacija pride iz vrhnjega pravila označenega s črko \(u\).
Pomembno je, da \(A\), kasneje v dokazu ne moremo več uporabiti, namreč predpostavili smo \(A\) samo zato da lahko dokažemo \(A \implies B\), vendar \(A\) ne drži nujno na splošno. Zato takoj ko uporabimo pravilo \(I^u\), se veja drevesa od \(u\) do \(I^u\) zapre. Nikjer drugje ne moremo uporabiti več sodb, ki smo jih dobili pod pogojem da smo predpostavili \(A\)
...
In spet oznaka \(E^{u, v}\) pove, da ne moremo uporabiti sodb na vejah med \(u\) in \(E^{u, v}\) in med \(w\) in \(E^{u, v}\) nikjer drugje.
